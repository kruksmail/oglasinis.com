...
class Oglas_AdminController extends Oglas_Library_Controller_Action_Abstract {
	/**
	...
	public function adminizmenaAction() {
		...
		if ($userRole == 'Administrator') {
			...
			//			$formaUnosOglasa->removeElement ( 'slika4' );
			...
			if ($this->getRequest ()->isPost ()) {
				if ($formaUnosOglasa->isValid ( $_POST )) {
					...
					$proveren = $formaUnosOglasa->getValue ( 'proveren' );
					...
					return $this->_redirect ( 'oglas/admin/' );
				}
			}
	...
	public function listaadminAction() {
		...
		$adapter = $oglasModel->getAllForAdmin ( $kriterijum );
	...
	public function adminbrisanjeAction() {
		$oglasID = $this->_request->getParam ( 'oglas' );
		$itemOglas = new Oglas_Model_Oglas ();
		$itemOglas->deleteOglas ( $oglasID );
		return $this->_redirect ( 'oglas/admin/listaadmin' );
	}
	public function rebuildemailsAction() {
		$oglasModel = new Oglas_Model_Oglas ();
		$oglasData = $oglasModel->getAllEmails ();
		$i=0;
		foreach ( $oglasData as $key => $value ) {
			
			if (! $value->email == "") {
				$emailTemp = new Oglas_Model_Emailnotification ();
				//echo $value->email ;
				$count = $emailTemp->checkEmail ( $value->email );
				//echo gettype( $value->email);
				if ($count==0) {
					$emailTemp->createFullEmail ( $value->email, "oglas" );
					$i++;
				}
		...
		$this->view->ukupno=$emailTemp->countAllEmails();
		$this->view->ukupnododato=$i;
		$this->view->svimasesalje=$emailTemp->countSvimaNotification();
		$this->view->nista=$emailTemp->countNistaNotification();
		$this->view->custom=$emailTemp->countCustomNotification();
	}
